...
export const auth = (email, password) => {
	return dispatch => {
		...
		const queryParams = "?grant_type=password&username=" + email + "&password=" + password;
		const config = {
			headers: {
				authorization:
		...
		axios
			...
			.then(response => {
				...
				dispatch(authSuccessStart(response.data.access_token, response.data.token_type));
